...
class Locker {
   ...
   float strokeW = 1;
   Locker (float posx, float posy, float size) {
     /* initiate Locker
      * 
  ...
  void setLocker (color outer, color inner) {
    outerCol = outer;
    innerCol = inner;
    strokeW = 0;
  } 
...
void setup() {
  ...
  float posY = 10;
  float margin = (width - numLockers * lockerSize)/2;
  
  for (int pos = 0; pos < numLockers; pos++) {
    float posX = margin + pos * lockerSize;
    lockers[pos] = new Locker(posX, posY, lockerSize);
  }
}
...
void draw() {
  ...
  // set the colors of the current locker
  ...
  lockers[currentLocker].setLocker(handleColor, complementryColor);
  
  // draw the lockers
  
  for (int i=0; i < numLockers; i++) {
    lockers[i].drawLocker();
  }  
}
